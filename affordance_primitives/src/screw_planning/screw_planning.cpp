...
//const auto screw_in_starting = transformScrew(screw_in_planning, starting_pose);
  ...
  screw_axis.setScrewAxis(screw_in_planning);
  ...
  const ScrewAxis & screw_axis)
{
  ...
  const Eigen::Isometry3d tf_q_to_path = tf_m_to_q.inverse() * screw_axis.getTF(current_theta) * tf_m_to_e;
  ...
  Eigen::Ref<Eigen::VectorXd> out)
{
  ...
  const auto closest_pt = findClosestPoint(current_pose, start_pose, theta_guess, theta_max, screw_axis);

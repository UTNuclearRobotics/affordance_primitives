...
double calcErrorDerivative(
  ...
  const std::vector<double> phi_current, const std::vector<ScrewAxis> & screw_axis_set)
{
//Algorithm
...
//Compute tf_q_to_p
  const Eigen::Isometry3d pOE = productOfExponentials (screw_axis_set, phi_current, screw_axis_set.size(), 0);
...
//Construct E and psi matrices
  ...
  Eigen::MatrixXd Ej(1,m);
  Eigen::MatrixXd psi(m,1);

for (int j=0; j<=screw_axis_set.size(); j++)
{
Ej << xi.transpose(), Eigen::MatrixXd::Zero(1,m-6*j);
...
bool constraintFn(
  const Eigen::Isometry3d & tf_m_to_q, const Eigen::Isometry3d & tf_m_to_s, const std::vector<ScrewAxis> & screwAxisSet, std::vector<double> phi_max, std::vector<double> phi_guess,
...
if (size > 1) && (start<=end)
    return screwAxisSet[start].getTF(phi[start])*productOfExponentials(screwAxisSet,size-1,start+1, end);
else
